...
<body>
    ...
    <script>
        ...
        function loadDemoAgendaItems() {
            const agendaItems = [
                { id: 1, title: 'Açılış ve Yoklama', responsible: 'Başkan', notes: 'Toplantıya katılan üyelerin tespiti' },
                { id: 2, title: 'Geçen Toplantı Kararlarının Değerlendirilmesi', responsible: 'Sekreter', notes: 'Önceki toplantıda alınan kararların takibi' },
                { id: 3, title: 'Mali Durum Raporu', responsible: 'Muhasebe Sorumlusu', notes: 'BYK bütçesi ve harcamaların değerlendirilmesi' },
                { id: 4, title: 'Eğitim Çalışmaları', responsible: 'Eğitim Sorumlusu', notes: 'Planlanan eğitim programları ve etkinlikler' },
                { id: 5, title: 'Yeni Dönem Planlaması', responsible: 'Başkan', notes: 'Gelecek dönem için stratejik planlar' }
            ];
                    ...
                    <div class="agenda-item mb-3 p-3 border rounded">
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1">
                                <h6 class="mb-1">${index + 1}. ${item.title}</h6>
                                <p class="text-muted mb-1"><strong>Sorumlu:</strong> ${item.responsible}</p>
                                <p class="text-muted mb-0">${item.notes}</p>
                </div>
                            <div class="btn-group btn-group-sm">
                                <button class="btn btn-outline-success" onclick="markAgendaCompleted(${item.id})" title="Tamamlandı">
                                    <i class="fas fa-check"></i>
                                </button>
                                <button class="btn btn-outline-primary" onclick="editAgendaItem(${item.id})" title="Düzenle">
                                    <i class="fas fa-edit"></i>
                    </button>
                </div>
                        </div>
        ...
        function loadDemoParticipants() {
            const participants = [
                { name: 'Ahmet Yılmaz', role: 'Başkan', status: 'present' },
                { name: 'Fatma Demir', role: 'Sekreter', status: 'present' },
                { name: 'Mehmet Çelik', role: 'Üye', status: 'present' },
                { name: 'Ayşe Kaya', role: 'Üye', status: 'absent' },
                { name: 'Zeynep Özkan', role: 'Üye', status: 'present' }
            ];
            ...
            participants.forEach(participant => {
                const participantHtml = `
                    <div class="d-flex align-items-center mb-2 p-2 border rounded">
                        ...
                        <span class="badge bg-${participant.status === 'present' ? 'success' : 'danger'}">
                            ${participant.status === 'present' ? 'Katıldı' : 'Katılmadı'}
                        </span>
                    </div>
                `;
