...
import { getRezervasyonTalepleriAction, actionRezervasyonTalebi } from "../../actions/auth";
...
const subOptionFields: Record<string, any[]> = {
  otel: [
    { name: "Giriş Tarihi", type: "date", key: "checkin" },
    { name: "Çıkış Tarihi", type: "date", key: "checkout" },
    { name: "Kişi Sayısı", type: "number", key: "guests" }
  ],
  ucak: [
    { name: "Gidiş Tarihi", type: "date", key: "departure_date" },
    { name: "Dönüş Tarihi", type: "date", key: "return_date" },
    { name: "Nereden", type: "text", key: "from" },
    { name: "Nereye", type: "text", key: "to" }
  ],
  seminer: [
    { name: "Toplantı Tarihi", type: "date", key: "event_date" },
    { name: "Saat", type: "time", key: "time" },
    { name: "Katılımcı Sayısı", type: "number", key: "attendees" }
  ],
  araba: [
    { name: "Alış Tarihi", type: "date", key: "pickup_date" },
    { name: "İade Tarihi", type: "date", key: "return_date" },
    { name: "Araç Tipi", type: "text", key: "car_type", placeholder: "Örn: Ekonomi, SUV..." }
  ],
  genel: []
};

export default function RezervasyonTalepleriPage() {
  ...
  async function loadRequests() {
    setLoading(true);
    const res = await getRezervasyonTalepleriAction({ tab: activeTab, durum: durumFilter });
    if (res.success) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    // Merge subOptions into a structured description or meta
    const subDetails = Object.entries(subOptions)
      .filter(([_, v]) => v)
      .map(([k, v]) => {
        const field = [...(subOptionFields[kategori] || []), ...subOptionFields.genel].find(f => f.key === k);
        return `${field?.name || k}: ${v}`;
      }).join('\n');
      
    const combinedAciklama = `${aciklama}${subDetails ? '\n\nDetaylar:\n' + subDetails : ''}`;

    const res = await actionRezervasyonTalebi({ 
      action: "yeni_harcama", 
      baslik, 
      tutar: 0, 
      kategori, 
      aciklama: combinedAciklama 
    });
    
    if (res.success) {
      showMessage(res.message, "success");
      setBaslik(""); setKategori("genel"); setAciklama(""); setSubOptions({});
      await loadRequests();
    } else {
  ...
  const handleAction = async (actionStr: string, id: number, extraData: any = {}) => {
    ...
    const res = await actionRezervasyonTalebi({ action: actionStr, talep_id: id, ...extraData });
    if (res.success) {
      showMessage(res.message, "success");
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              ...
              <textarea
                ...
                placeholder="Örn: Bu tarih aralığı dolu..."
                ...
                <form onSubmit={handleCreate} className="p-5 space-y-4">
                  ...
                  <div className="space-y-1.5 flex flex-col">
                    ...
                    <select value={kategori} onChange={(e) => {
                      ...
                      setSubOptions({});
                    }} required className="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-zinc-200 focus:border-emerald-500/50 focus:ring-1 outline-none">
                  ...
                  {/* Dynamic Sub-options */}
                  {subOptionFields[kategori]?.map(field => (
                    <div key={field.key} className="space-y-1.5 flex flex-col">
                      <label className="text-xs font-semibold text-zinc-400">{field.name}</label>
                      <input 
                        type={field.type} 
                        placeholder={field.placeholder}
                        value={subOptions[field.key] || ""} 
                        onChange={(e) => setSubOptions({...subOptions, [field.key]: e.target.value})} 
                        className="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-zinc-200 focus:border-emerald-500/50 focus:ring-1 outline-none" 
                      />
                    </div>
                  ))}

                  <div className="space-y-1.5 flex flex-col">
                    <label className="text-xs font-semibold text-zinc-400">Ek Notlar</label>
                    <textarea rows={3} value={aciklama} onChange={(e) => setAciklama(e.target.value)} className="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-zinc-200 focus:border-emerald-500/50 focus:ring-1 outline-none resize-none" placeholder="Varsa eklemek istediğiniz detaylar..."></textarea>
                  </div>
                  <button type="submit" disabled={submitLoading || !baslik} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-medium py-3 rounded-xl transition-all flex items-center justify-center gap-2">
                      ...
                      return (
                        <div key={req.talep_id} className="bg-zinc-950 border border-white/5 rounded-xl p-4 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                          <div className="flex-1">
                            ...
                            {req.kisa_aciklama && <p className="text-xs text-zinc-500 mt-2 whitespace-pre-line">{req.kisa_aciklama}</p>}
                    ...
                    <div key={req.talep_id} className="bg-zinc-900 border border-white/5 rounded-2xl p-5 hover:border-emerald-500/20 transition-all flex flex-col justify-between">
                      <div>
                        ...
                        <div className="space-y-3 mb-4">
                          ...
                          {req.kisa_aciklama && (
                             <div className="bg-zinc-950 p-2.5 rounded-xl border border-white/5 text-xs text-zinc-400 whitespace-pre-line">
                               <span className="text-zinc-500 font-semibold block mb-1">Talep Detayları:</span>
                               {req.kisa_aciklama}
                             </div>
                          )}
