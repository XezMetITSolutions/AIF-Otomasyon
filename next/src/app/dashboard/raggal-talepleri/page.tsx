    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-zinc-400">Raggal toplantı odası veya tesis rezervasyonlarını izleyin.</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5 bg-zinc-900/80">
                ...
                <th className="p-4 text-xs font-semibold text-zinc-400">Oda/Tesis</th>
                <th className="p-4 text-xs font-semibold text-zinc-400">Saat</th>
            ...
            <tbody>
              {requests.map((r) => (
                <tr key={r.id || r.talep_id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-4 text-sm text-zinc-300">{r.oda_adi || "Toplantı Salonu"}</td>
                  <td className="p-4 text-sm text-zinc-300">{r.saat_araligi || "09:00 - 12:00"}</td>
                  <td className="p-4">
