...
class PermissionManager {
    ...
    /**
     * Kullanıcının görüntüleme yetkisi var mı (read/manager)
     */
    ...
    /**
     * Kullanıcının düzenleme yetkisi var mı (sadece manager)
     */
    public static function canEdit(string $username, string $module): bool {
        // Model iki katman: Üye(read) sadece görüntüleme; Müdür(manager) görüntüleme+düzenleme
        return self::canAdmin($username, $module);
    }

    /**
     * Sayfa erişim koruması: minimum seviye enforce ("read" | "manager")
     * Yoksa role bazlı panel veya giriş sayfasına yönlendirir.
     */
    public static function requireModuleAccess(string $module, string $minLevel = 'read'): void {
        // Oturumdan kullanıcıyı çek
        ...
        $ok = false;
        if ($minLevel === 'read') {
            $ok = in_array($level, ['read', 'write', 'manager'], true);
        } elseif ($minLevel === 'manager') {
            $ok = ($level === 'manager');
        }

        if (!$ok) {
            // Yetersiz yetki → role bazlı yönlendirme
