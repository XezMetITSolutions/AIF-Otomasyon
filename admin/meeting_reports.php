...
<body>
    ...
    <script>
        ...
        // Eksik JavaScript fonksiyonları
        function deleteMeeting(id) {
            if (confirm('Bu toplantıyı silmek istediğinizden emin misiniz?')) {
                fetch('api/meeting_api.php?action=delete_meeting', {
                    method: 'POST',
        ...
        function viewMeeting(id) {
            // Toplantı detaylarını modal'da göster
            fetch('api/meeting_api.php?action=get_meeting&id=' + id)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        // Toplantı detaylarını göster
                        showAlert('Toplantı detayları: ' + data.data.title, 'info');
                    } else {
                        showAlert('Hata: ' + data.message, 'danger');
                    }
                })
        ...
        function editMeeting(id) {
            // Toplantı verilerini al ve modal'ı düzenleme modunda aç
            fetch('api/meeting_api.php?action=get_meeting&id=' + id)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('meetingBYK').value = meeting.byk_code || '';
                        ...
                        document.getElementById('meetingUnit').value = meeting.unit || '';
                        document.getElementById('meetingAgenda').value = meeting.agenda || '';
                        
                        // Formu edit moduna al
                        const form = document.getElementById('addMeetingForm');
                        form.setAttribute('data-edit-id', id);
                        
                        // Modal başlığını değiştir
                        document.getElementById('addMeetingModalLabel').textContent = 'Toplantı Düzenle';
