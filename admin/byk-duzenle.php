<?php
/**
 * Ana Yönetici - BYK Düzenleme (byk_categories tablosu)
 */
...
try {
    ...
    if (!$byk) {
        header('Location: /admin/byk.php?error=notfound');
        exit;
    }
} catch (Exception $e) {
    header('Location: /admin/byk.php?error=notfound');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($code)) {
        ...
        // Aynı ID hariç kodu kontrol et
        $existing = $db->fetch("SELECT id FROM byk_categories WHERE code = ? AND id != ?", [$code, $id]);
        if ($existing) {
    ...
    if (empty($errors)) {
        try {
            // byk_categories tablosunu güncelle
            $db->query("
                UPDATE byk_categories 
                SET code = ?, name = ?, color = ?, description = ?, updated_at = NOW()
                WHERE id = ?
            ...
            $errors[] = 'BYK güncellenirken bir hata oluştu: ' . $e->getMessage();
        }
    }
}
...
<main class="container-fluid mt-4">
    <div class="content-wrapper">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3 mb-0">
                <i class="fas fa-edit me-2"></i>BYK Düzenle
            </h1>
                ...
                <form method="POST">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="code" name="code" 
                                   value="<?php echo htmlspecialchars($_POST['code'] ?? $byk['code']); ?>" 
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                <input type="color" class="form-control form-control-color" id="colorPicker" 
                                       value="<?php echo htmlspecialchars($_POST['color'] ?? $byk['color'] ?? '#009872'); ?>" 
                                       title="Renk seçin">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="name" name="name" 
                               value="<?php echo htmlspecialchars($_POST['name'] ?? $byk['name']); ?>" 
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="3" 
                                  placeholder="BYK hakkında açıklama..."><?php echo htmlspecialchars($_POST['description'] ?? $byk['description'] ?? ''); ?></textarea>
                    </div>
