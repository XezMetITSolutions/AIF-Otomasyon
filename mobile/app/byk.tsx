...
import { fetchBYK } from '@/services/api';

export default function BYKScreen() {
  ...
  const loadData = async () => {
    setLoading(true);
    const result = await fetchBYK();
    if (result.success) setData(result.byk);
    setLoading(false);
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <Stack.Screen options={{ title: 'BYK Yönetimi' }} />
      {loading && !refreshing ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
               <View style={[styles.iconBox, { backgroundColor: (item.renk_kodu || '#6366f1') + '15' }]}>
                <FontAwesome6 name="building" color={item.renk_kodu || '#6366f1'} size={18} />
              </View>
              <View style={styles.info}>
                <Text style={styles.title}>{item.byk_adi}</Text>
                <Text style={styles.subtitle}>{item.byk_kodu}</Text>
              </View>
