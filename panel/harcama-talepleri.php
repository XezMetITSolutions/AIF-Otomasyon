 ...
 * 
 * Onay Akışı:
 * 1. Seviye: Yasin Çakmak (İlk Onay)
 * 2. Seviye: Muhammed Enes Sivrikaya - AT Muhasebe Başkanı (İkinci Onay)
 */
...
// Kullanıcının onaylayıcı olup olmadığını kontrol et
...
if (!empty($harcamaWorkflow['first_approver_user_id']) && $user['id'] == $harcamaWorkflow['first_approver_user_id']) {
    ...
    $hasPermissionBaskan = true; // İlk onaylayıcıya başkan yetkisi ver
}

if (!empty($harcamaWorkflow['second_approver_user_id']) && $user['id'] == $harcamaWorkflow['second_approver_user_id']) {
    $isSecondApprover = true;
    $hasPermissionBaskan = true; // İkinci onaylayıcıya başkan yetkisi ver
}
...
$durumBadgeMap = [
    'beklemede' => 'warning',        // Beklemede (henüz kimse onaylamadı)
    'ilk_onay' => 'info',          // İlk onaydan geçti (Yasin onayladı), ikinci onay bekliyor
    'onaylandi' => 'success',      // Tamamen onaylandı (her iki onaydan da geçti)
    'reddedildi' => 'danger',      // Reddedildi
    'odenmistir' => 'primary'      // Ödendi
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Middleware::verifyCSRF()) {
        ...
        if ($action === 'yeni_harcama' && $hasPermissionUye) {
            ...
            if (!array_key_exists($kategori, $kategoriListesi))
                $errors[] = 'Geçersiz kategori.';

            if ($kategori === 'seyahat') {
                if ($seyahat_cikis === '' || $seyahat_varis === '')
                    $errors[] = 'Güzergâh bilgisi zorunludur.';
                if (!$seyahat_gidis || !$seyahat_donus)
                    $errors[] = 'Tarihler zorunludur.';
                elseif ($seyahat_donus < $seyahat_gidis)
                    $errors[] = 'Dönüş tarihi hatalı.';
            }
            ...
            // 2. Onay/Red (Başkan veya Onaylayıcılar)
        } elseif (($action === 'approve' || $action === 'reject') && ($hasPermissionBaskan || $isFirstApprover || $isSecondApprover)) {
            ...
            if (!$talep) {
                ...
                // İki seviyeli onay mantığı
                $currentLevel = $talep['onay_seviyesi'] ?? 0;
                $currentStatus = $talep['durum'];

                // Red işlemi - herhangi bir seviyede reddedilebilir
                if ($action === 'reject') {
                    if ($currentStatus === 'reddedildi' || $currentStatus === 'onaylandi') {
                        $errors[] = 'Bu talep zaten sonuçlandırılmış.';
                    } else {
                        // Hangi seviyede reddedildiğini kaydet
                        if ($isFirstApprover || ($currentLevel == 0 && $hasPermissionBaskan)) {
                            // İlk onaylayıcı red etti
                            $db->query("
                                ...
                                SET durum = 'reddedildi',
                                    ...
                                    onay_seviyesi = 0
                                WHERE talep_id = ?
                            ", [$user['id'], $aciklama ?: 'Talep reddedildi.', $talepId]);
                            $messages[] = 'Talep reddedildi (1. Onaylayıcı).';
                        } elseif ($isSecondApprover || ($currentLevel == 1 && $hasPermissionBaskan)) {
                            // İkinci onaylayıcı red etti
                            $db->query("
                                UPDATE harcama_talepleri
                                SET durum = 'reddedildi',
                                    ikinci_onaylayan_id = ?,
                                    ikinci_onay_tarihi = NOW(),
                                    ikinci_onay_aciklama = ?
                                WHERE talep_id = ?
                            ", [$user['id'], $aciklama ?: 'Talep reddedildi.', $talepId]);
                            $messages[] = 'Talep reddedildi (2. Onaylayıcı).';
                        }
                    ...
                    if ($isFirstApprover && $currentLevel == 0 && $currentStatus === 'beklemede') {
                        $db->query("
                            ...
                            SET durum = 'ilk_onay',
                                ...
                                ilk_onay_aciklama = ?
                            WHERE talep_id = ?
                        ", [$user['id'], $aciklama ?: 'İlk onay verildi.', $talepId]);

                        $messages[] = 'İlk onay tamamlandı. Talep ikinci onaylayıcıya (AT Muhasebe Başkanı) gönderildi.';
                        ...
                        $db->query("
                            ...
                            SET durum = 'onaylandi',
                                ...
                                ikinci_onay_aciklama = ?
                            WHERE talep_id = ?
                        ", [$user['id'], $aciklama ?: 'İkinci onay verildi. Talep tamamen onaylandı.', $talepId]);

                        $messages[] = 'İkinci onay tamamlandı. Talep tamamen onaylandı!';
                        ...
                        // Başkan tam yetkili - direkt onaylayabilir
                    } elseif ($hasPermissionBaskan && !$isFirstApprover && !$isSecondApprover) {
                        $db->query("
                            UPDATE harcama_talepleri
                            SET durum = 'onaylandi',
                                onay_seviyesi = 2,
...
if ($hasPermissionBaskan || $isFirstApprover || $isSecondApprover) {
    $filters = ['ht.byk_id = ?'];
    $params = [$user['byk_id']];
    
    // İlk onaylayıcı - Sadece beklemede olan talepleri göster
    if ($isFirstApprover && !$isSecondApprover && !$hasPermissionBaskan) {
        if (!$durumFilter) {
            $durumFilter = 'beklemede';
        }
    }
    
    // İkinci onaylayıcı - Sadece ilk onaydan geçmiş talepleri göster
    if ($isSecondApprover && !$isFirstApprover && !$hasPermissionBaskan) {
        if (!$durumFilter) {
            $durumFilter = 'ilk_onay';
        }
    }
                     ...
                     <div class="card border-0 shadow-sm">
                        <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <div class="btn-group">
                                ...
                                <a href="?tab=onay&durum=ilk_onay" class="btn btn-sm <?php echo $durumFilter === 'ilk_onay' ? 'btn-info' : 'btn-outline-info'; ?>">
                                    <i class="fas fa-check-circle me-1"></i>İlk Onay
                                </a>
                                <a href="?tab=onay&durum=onaylandi" class="btn btn-sm <?php echo $durumFilter === 'onaylandi' ? 'btn-success' : 'btn-outline-success'; ?>">
                                    <i class="fas fa-check-double me-1"></i>Onaylandı
                                </a>
                                        ...
                                        <table class="table table-hover align-middle mb-0">
                                            ...
                                            <tbody>
                                                <?php foreach ($pendingRequests as $req):
                                                    ...
                                                    $catLabel = $kategoriListesi[$meta['kategori'] ?? 'genel'] ?? 'Genel';
                                                    $currentLevel = $req['onay_seviyesi'] ?? 0;
                                                    ?>
                                                    <tr>
                                                        <td class="ps-3">
                                                        ...
                                                        <td>
                                                            <div class="d-flex flex-column gap-1">
                                                                <span class="badge bg-<?php echo $badge; ?> bg-opacity-10 text-<?php echo $badge; ?> px-2 py-1 border border-<?php echo $badge; ?> border-opacity-10">
                                                                    <?php 
                                                                    if ($req['durum'] === 'beklemede') echo 'Beklemede';
                                                                    elseif ($req['durum'] === 'ilk_onay') echo 'İlk Onay ✓';
                                                                    elseif ($req['durum'] === 'onaylandi') echo 'Onaylandı ✓✓';
                                                                    elseif ($req['durum'] === 'reddedildi') echo 'Reddedildi';
                                                                    else echo ucfirst($req['durum']);
                                                                    ?>
                                                                </span>
                                                                <?php if ($currentLevel >= 1 && !empty($req['ilk_onaylayan_ad'])): ?>
                                                                    <small class="text-muted" style="font-size: 0.7rem;">
                                                                        <i class="fas fa-user-check text-info"></i> <?php echo htmlspecialchars($req['ilk_onaylayan_ad']); ?>
                                                                    </small>
                                                                <?php endif; ?>
                                                                <?php if ($currentLevel >= 2 && !empty($req['ikinci_onaylayan_ad'])): ?>
                                                                    <small class="text-muted" style="font-size: 0.7rem;">
                                                                        <i class="fas fa-user-check text-success"></i> <?php echo htmlspecialchars($req['ikinci_onaylayan_ad']); ?>
                                                                    </small>
                                                                <?php endif; ?>
                                                            </div>
                                                        </td>
                                                        <td class="text-end pe-3">
                                                            <div class="btn-group">
                                                                <?php if ($req['durum'] === 'beklemede' && ($isFirstApprover || $hasPermissionBaskan)): ?>
                                                                    <!-- İlk onaylayıcı için butonlar -->
                                                                    <form method="post" onsubmit="return confirm('İlk onayı veriyor musunuz? Talep ikinci onaylayıcıya gönderilecek.');">
                                                                        <input type="hidden" name="<?php echo $csrfTokenName; ?>" value="<?php echo $csrfToken; ?>">
                                                                        <input type="hidden" name="action" value="approve">
                                                                        <input type="hidden" name="talep_id" value="<?php echo $req['talep_id']; ?>">
                                                                        <button class="btn btn-sm btn-info" type="submit" title="İlk Onay"><i class="fas fa-check"></i> 1. Onay</button>
                                                                    </form>
                                                                ...
                                                                <?php elseif ($req['durum'] === 'ilk_onay' && ($isSecondApprover || $hasPermissionBaskan)): ?>
                                                                    <!-- İkinci onaylayıcı için butonlar -->
                                                                    <form method="post" onsubmit="return confirm('İkinci onayı veriyor musunuz? Talep tamamen onaylanacak.');">
                                                                        <input type="hidden" name="<?php echo $csrfTokenName; ?>" value="<?php echo $csrfToken; ?>">
                                                                        <input type="hidden" name="action" value="approve">
                                                                        <input type="hidden" name="talep_id" value="<?php echo $req['talep_id']; ?>">
                                                                        <button class="btn btn-sm btn-success" type="submit" title="İkinci Onay"><i class="fas fa-check-double"></i> 2. Onay</button>
                                                                    </form>
                                                                    <button class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#rejectModal" data-id="<?php echo $req['talep_id']; ?>" title="Reddet">
                                                                        <i class="fas fa-times"></i>
                                                                    </button>
                                                                ...
                                                                <?php if ($hasPermissionBaskan): ?>
                                                                    <!-- Başkan / Admin edit / delete butonları -->
