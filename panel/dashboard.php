...
// Şube Ziyaretleri Entegrasyonu
if ($auth->hasModulePermission('baskan_sube_ziyaretleri')) {
    ...
    $yaklasan_ziyaretler = $db->fetchAll("
        SELECT z.*, 
               b.byk_adi as target_byk,
               g.grup_adi,
               g.renk_kodu
        FROM sube_ziyaretleri z
        JOIN byk b ON z.byk_id = b.byk_id
        JOIN ziyaret_gruplari g ON z.grup_id = g.grup_id
        WHERE " . implode(' AND ', $visitWhere) . "
        ORDER BY z.ziyaret_tarihi ASC
        LIMIT 5
    ", $visitParams);

    foreach ($yaklasan_ziyaretler as $z) {
        $yaklasan_etkinlikler[] = [
            'baslik' => $z['target_byk'] . ' Şube Ziyareti', // Başlık: "X Şubesi Ziyareti"
            'baslangic_tarihi' => $z['ziyaret_tarihi'],
            'byk_adi' => $z['grup_adi'], // Alt Metin: Grup Adı
            'byk_renk' => $z['renk_kodu'],
            'type' => 'ziyaret',
            'id' => $z['ziyaret_id']
        ];
    }
...
usort($yaklasan_etkinlikler, function ($a, $b) {
    return strtotime($a['baslangic_tarihi']) - strtotime($b['baslangic_tarihi']);
});
$yaklasan_etkinlikler = array_slice($yaklasan_etkinlikler, 0, 7); // Toplam 7 kayıt göster
    ...
    <main class="main-content">
        <div class="content-wrapper">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h1 class="h3 mb-0">
                        <i class="fas fa-user-circle me-2"></i>Hoş Geldiniz,
                ...
                <div class="d-none d-lg-flex align-items-center gap-3 customize-section">
                    <button type="button" class="btn btn-outline-secondary btn-sm rounded-pill px-3"
                        data-bs-toggle="modal" data-bs-target="#dashboardSettingsModal">
                            ...
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li><a class="dropdown-item <?php echo $eventScope === 'all' ? 'active' : ''; ?>"
                                                href="?event_scope=all">Tümü</a></li>
                                        <li><a class="dropdown-item <?php echo $eventScope === 'my_unit' ? 'active' : ''; ?>"
                                                href="?event_scope=my_unit">Birimim</a></li>
                                    </ul>
                                </div>
                            </div>
                                ...
                                <?php else: ?>
                                    <div class="list-group list-group-flush">
                                        <?php foreach ($yaklasan_etkinlikler as $etkinlik): ?>
                                            ...
                                            // Icon Belirle
                                            $icon = isset($etkinlik['type']) && $etkinlik['type'] === 'ziyaret'
                                                ? 'fa-map-marker-alt'
                                                : 'fa-sitemap';
                                            ?>
                                            <div class="list-group-item d-flex align-items-center justify-content-between">
                                                ...
                                                <div class="d-flex align-items-center">
                                                    <div class="rounded p-2 text-center me-3 text-white"
                                                        style="min-width: 50px; background-color: <?php echo $renk; ?>;">
                                                        <div class="fw-bold">
                                                            <?php echo date('d', strtotime($etkinlik['baslangic_tarihi'])); ?></div>
                                                        <div class="small text-uppercase" style="font-size: 0.65rem; opacity: 0.9;">
                                                            <?php echo $aylar[$ayKodu] ?? date('M', strtotime($etkinlik['baslangic_tarihi'])); ?>
                                                        </div>
                                                    </div>
                                                    <div>
                                                        ...
                                                        <small class="text-muted d-block mt-1">
                                                            <i class="fas <?php echo $icon; ?> me-1"
                                                                style="font-size: 0.8em; color: <?php echo $renk; ?>;"></i>
                                                            <?php echo htmlspecialchars($birimAdi); ?>
                                                        </small>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="d-flex align-items-center">
                                                    <div class="bg-light rounded p-2 text-center me-3" style="min-width: 50px;">
                                                        <div class="fw-bold text-dark">
                                                            <?php echo date('d', strtotime($toplanti['toplanti_tarihi'])); ?></div>
                                                        <div class="small text-muted text-uppercase" style="font-size: 0.65rem;">
                                                            <?php echo $aylar[$ayKodu] ?? date('M', strtotime($toplanti['toplanti_tarihi'])); ?>
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <h6 class="mb-1 fw-semibold">
                                                            <a href="/panel/toplantilar.php?id=<?php echo $toplanti['toplanti_id']; ?>"
                                                        ...
                                                        <div class="d-flex flex-column gap-1">
                                                            <small class="text-muted">
                                                                <i class="fas fa-sitemap me-1 text-success"
                                                            ...
                                                            <div class="d-flex gap-2">
                                                                <span class="badge bg-light text-dark border fw-normal py-1">
                                                                    <i
                                                                        class="far fa-clock me-1 small"></i><?php echo date('H:i', strtotime($toplanti['toplanti_tarihi'])); ?>
                                                                </span>
                                                                <?php
                                                                ...
                                                                ?>
                                                                <span
                                                                    class="badge bg-<?php echo $statusClass; ?> bg-opacity-10 text-<?php echo $statusClass; ?> border border-<?php echo $statusClass; ?> border-opacity-10 py-1">
                                                                    <?php echo $statusText; ?>
                                                                </span>
                                                            </div>
                                                        </div>
                                ...
                                <?php else: ?>
                                    <div class="list-group list-group-flush">
                                        <?php foreach ($son_duyurular as $duyuru): ?>
                                            <a href="/panel/duyurular.php?id=<?php echo $duyuru['duyuru_id']; ?>"
                                                class="list-group-item list-group-item-action d-flex align-items-center justify-content-between">
                                                <div>
                                                    <h6 class="mb-0 fw-semibold text-dark">
                                                        <?php echo htmlspecialchars($duyuru['baslik']); ?></h6>
                                                    <small class="text-muted text-truncate d-inline-block"
                                                        style="max-width:300px; font-size: 0.8em;"><?php echo mb_substr(strip_tags($duyuru['icerik']), 0, 80); ?>...</small>
                                                </div>
                                                <small class="text-muted ms-2"
                                                    style="white-space:nowrap; font-size:0.7em;"><?php echo date('d.m', strtotime($duyuru['olusturma_tarihi'])); ?></small>
                                            </a>
                                        <?php endforeach; ?>
                                    </div>
                                <?php endif; ?>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover align-middle mb-0">
                                        <thead class="bg-light">
                                            <tr>
                                                <th class="border-0 px-4 py-3 fw-semibold text-muted small text-uppercase">
                                                    Tarih</th>
                                                <th class="border-0 px-4 py-3 fw-semibold text-muted small text-uppercase">
                                                    İzin Nedeni</th>
                                                <th class="border-0 px-4 py-3 fw-semibold text-muted small text-uppercase">
                                                    Durum</th>
                                                <th
                                                    class="border-0 px-4 py-3 fw-semibold text-muted small text-uppercase text-end">
                                                    İşlem</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                                    ...
                                                    <tr>
                                                        <td class="px-4">
                                                            <div class="d-flex flex-column">
                                                                <span
                                                                    class="fw-medium text-dark"><?php echo date('d.m.Y', strtotime($izin['baslangic_tarihi'])); ?></span>
                                                                <span
                                                                    class="small text-muted"><?php echo date('d.m.Y', strtotime($izin['bitis_tarihi'])); ?></span>
                                                            </div>
                                                        </td>
                                                        <td class="px-4 text-muted">
                                                            <?php echo htmlspecialchars($izin['izin_nedeni'] ?? '-'); ?></td>
                                                        <td class="px-4">
                                                            <?php
                                                            $durumRenk = [
                                                                'beklemede' => 'warning',
                                                            ...
                                                            ?>
                                                            <span
                                                                class="badge bg-<?php echo $renk; ?> bg-opacity-10 text-<?php echo $renk; ?> border border-<?php echo $renk; ?> border-opacity-10 px-3 py-2 rounded-pill">
                                                                <?php echo $durumText[$izin['durum']] ?? $izin['durum']; ?>
                                                            </span>
                                                        </td>
                                                        <td class="px-4 text-end">
                                                            <a href="/panel/uye_izin-talepleri.php?id=<?php echo $izin['izin_id']; ?>"
                                                                class="btn btn-sm btn-light rounded-pill px-3">
                                                                Detay
                                                            </a>
                                                        </td>
                        ...
                        <?php else: ?>
                            <?php foreach ($son_harcamalar as $h): ?>
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    <div>
                                        <div class="fw-semibold text-dark"><?php echo number_format($h['tutar'], 2); ?>
                                            <?php echo $h['para_birimi']; ?></div>
                                    ...
                                    ?>
                                    <span
                                        class="badge bg-<?php echo $stClass; ?> bg-opacity-10 text-<?php echo $stClass; ?> border border-<?php echo $stClass; ?> border-opacity-10 py-1"><?php echo $stText; ?></span>
                                </li>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </ul>
